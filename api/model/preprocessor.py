...
class PreProcessor:
    """Preprocess data for the ML model. Can split dataset in a holdout. Can scale the data input before prediction."""

    def split_test_train(self, dataset, test_percentage, seed=7):

        X_train, X_test, Y_train, Y_test = self.__set_holdout(
    ...
    def scaler(X_train):
        scaler_path = os.path.join(
            "machine_learning", "scalers", "standard_scaler_recruitment.pkl"
        )

        scaler = pickle.load(scaler_path, "rb")
